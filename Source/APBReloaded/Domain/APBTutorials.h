#pragma once
// APB TUTORIAL / "City Guide" catalog — the retail id -> {Title, SubTitle, Body} dictionary for the in-game
// new-player onboarding help system (the tutorial book / City Guide UI: "Welcome to San Paro", "Basic",
// "Movement & Actions", ...). Extracted from the retail Tutorials.INT (mirror of the cooked SDD table
// "Tutorials") by tools/scripts/extract_tutorials.ps1 -> Content/Data/tutorials.json.
//
// The Body carries the game's own lightweight HTML markup (<br>, <b>, <img>, ...) kept VERBATIM so the UE5
// UI can render it exactly like retail (1:1 fidelity). Of the 120 retail ids the "None" DO-NOT-TRANSLATE
// placeholder is dropped (none present here); all 120 carry a heading + tagline + body.
//
...
struct TutorialEntry {
	std::string id;       // "Root", "Basic", "MovementActions", ...
	std::string title;    // short heading ("Welcome to San Paro")
	std::string subtitle; // one-line tagline ("City Guide")
	std::string body;     // multi-paragraph HTML body (verbatim; may contain <br>, <b>, ...)
	int32_t order = 0;    // stable display order (file order)
};

class TutorialCatalog {
public:
	std::vector<TutorialEntry> items; // sorted by order
	...
	bool LoadFromJsonText(const std::string& text) {
		...
		for (const std::string& obj : SplitTopObjects(text)) {
			...
			r.subtitle = Unescape(RawStr(obj, "subtitle"));
			r.body     = Unescape(RawStr(obj, "body"));
			r.order    = (int32_t)RNum(obj, "order", 0);
			if (r.id.empty()) continue;
			auto it = std::find_if(items.begin(), items.end(),
				[&](const TutorialEntry& e){ return e.id == r.id; });
			if (it == items.end()) items.push_back(r);
			else *it = r;
			++touched;
		}
		std::sort(items.begin(), items.end(),
			[](const TutorialEntry& a, const TutorialEntry& b){ return a.order < b.order; });
		return touched > 0;
	}

	const TutorialEntry* Find(const std::string& id) const {
		for (const auto& r : items) if (r.id == id) return &r;
		return nullptr;
	}

	std::string Title(const std::string& id, const std::string& def = std::string()) const {
		const TutorialEntry* r = Find(id);
		return r ? r->title : def;
	}

	std::string SubTitle(const std::string& id, const std::string& def = std::string()) const {
		const TutorialEntry* r = Find(id);
		return r ? r->subtitle : def;
	}
	...
	// True if the tutorial exists AND carries a non-empty body.
	bool HasBody(const std::string& id) const {
		const TutorialEntry* r = Find(id);
		return r && !r->body.empty();
	}
