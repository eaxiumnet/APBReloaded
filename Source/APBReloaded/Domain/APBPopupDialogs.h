#pragma once
// APB POPUP DIALOG catalog — the retail id -> popup Body dictionary for the in-game advisory / help popups
// shown to the player during play ("Ammo: Locate an Ammo Vending Machine to resupply ammunition.",
// "Arrests: Enforcers can arrest you, however members of your team can free you.", vehicle-control hints,
// group/mission notices, ...). Extracted from the retail PopupDialogs.INT (mirror of the cooked SDD table
// "PopupDialogs") by tools/scripts/extract_popup_dialogs.ps1 -> Content/Data/popup_dialogs.json.
//
// Each id carries a category prefix (first '_'-token): AD (Advice/tutorial dialogs), TD, SD, RO, HUDO,
// GUI, etc. Of the retail ids the "None" DO-NOT-TRANSLATE placeholder is dropped; empty-body rows are
// also dropped (leaving 165). Some bodies embed UI markup tokens such as <Key:Handbrake> that the client
// resolves to the player's bound key at runtime; these are preserved VERBATIM for 1:1 rendering.
//
...
struct PopupDialogEntry {
	std::string id;    // "AD_AmmoLow_Chat", "TD_...", ...
	std::string body;  // the popup text (verbatim, may contain <Key:...> markup)
	int32_t order = 0; // stable display order (file order)
};

class PopupDialogCatalog {
public:
	std::vector<PopupDialogEntry> items; // sorted by order
	...
	bool LoadFromJsonText(const std::string& text) {
		...
		for (const std::string& obj : SplitTopObjects(text)) {
			...
			r.body  = Unescape(RawStr(obj, "body"));
			r.order = (int32_t)RNum(obj, "order", 0);
			if (r.id.empty()) continue;
			auto it = std::find_if(items.begin(), items.end(),
				[&](const PopupDialogEntry& e){ return e.id == r.id; });
			if (it == items.end()) items.push_back(r);
			else *it = r;
			++touched;
		}
		std::sort(items.begin(), items.end(),
			[](const PopupDialogEntry& a, const PopupDialogEntry& b){ return a.order < b.order; });
		return touched > 0;
	}

	const PopupDialogEntry* Find(const std::string& id) const {
		for (const auto& r : items) if (r.id == id) return &r;
		return nullptr;
	}

	std::string Body(const std::string& id, const std::string& def = std::string()) const {
		const PopupDialogEntry* r = Find(id);
		return r ? r->body : def;
	}

	// All popups belonging to a category (first '_'-separated token: "AD", "TD", "SD", ...), in display order.
	std::vector<const PopupDialogEntry*> ForCategory(const std::string& category) const {
		std::vector<const PopupDialogEntry*> out;
	...
	// The category token: the first '_'-separated segment ("AD_AmmoLow_Chat" -> "AD"). Ids with no '_'
