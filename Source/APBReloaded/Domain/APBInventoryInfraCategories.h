#pragma once
// APB inventory-item INFRASTRUCTURE-CATEGORY taxonomy — the category buckets the inventory / Armas /
// store UI uses to GROUP and LABEL items (Marketplace Cash, Character, Clothing: Accessories, Clothing:
// Armor (Vests), Weapons, Mods, Vehicles, Symbols, ...). Extracted from the retail
// InventoryItemInfraCategories.INT (mirror of the cooked SDD table "InventoryItemInfraCategories") by
// tools/scripts/extract_inventory_infra_categories.ps1 -> Content/Data/inventory_infra_categories.json.
//
// This is the categorisation/label layer OVER the master item-name dictionary in APBInventoryItemTypes.h
// (InventoryItemTypes.INT): each category carries a UI header (DisplayName), a plural/long label
// (Description) and a singular label (SingularName). The placeholder "None" id (all-empty) is dropped.
//
...
struct InventoryInfraCategory {
	std::string id;            // "MarketplaceCash", "ClothingAccessoryClothing", "ClothingArmourVests", ...
	std::string display_name;  // UI header, e.g. "Clothing: Accessories (Clothing)"
	std::string description;   // plural/long label, e.g. "Accessories (Clothing)"
	std::string singular_name; // singular label, e.g. "Accessory (Clothing)"
	int32_t order = 0;         // stable display order (file order)
};

class InventoryInfraCategoryCatalog {
public:
	std::vector<InventoryInfraCategory> items; // sorted by order
	...
	bool LoadFromJsonText(const std::string& text) {
		...
		for (const std::string& obj : SplitTopObjects(text)) {
			InventoryInfraCategory r;
			r.id            = Unescape(RawStr(obj, "id"));
			r.display_name  = Unescape(RawStr(obj, "display_name"));
			r.description   = Unescape(RawStr(obj, "description"));
			r.singular_name = Unescape(RawStr(obj, "singular_name"));
			r.order         = (int32_t)RNum(obj, "order", 0);
			if (r.id.empty()) continue;
			auto it = std::find_if(items.begin(), items.end(),
				[&](const InventoryInfraCategory& e){ return e.id == r.id; });
			if (it == items.end()) items.push_back(r);
			else *it = r;
			++touched;
		}
		std::sort(items.begin(), items.end(),
			[](const InventoryInfraCategory& a, const InventoryInfraCategory& b){ return a.order < b.order; });
		return touched > 0;
	}

	const InventoryInfraCategory* Find(const std::string& id) const {
	...
	std::string DisplayName(const std::string& id, const std::string& def = std::string()) const {
		const InventoryInfraCategory* r = Find(id);
	...
	std::string SingularName(const std::string& id, const std::string& def = std::string()) const {
		const InventoryInfraCategory* r = Find(id);
		return r ? r->singular_name : def;
	}

	// True if the category exists (every stored row has a non-empty display name).
