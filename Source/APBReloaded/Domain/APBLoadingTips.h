#pragma once
// APB LOADING-SCREEN TIP catalog — the retail id -> tip Message dictionary for the gameplay hints shown
// over the loading movie while a district streams in ("In the Customization Editors, press 1 to select the
// Move tool.", "Never give your account details or password to anyone else.", ...). Extracted from the
// retail LoadingMovieTips.INT (mirror of the cooked SDD table "LoadingMovieTips") by
// tools/scripts/extract_loading_tips.ps1 -> Content/Data/loading_tips.json.
//
// Each id carries a category prefix (first '_'-token): GP (Gameplay), SL, OW, UI, ED/EG (editors), etc.
// Of the retail ids the "None" DO-NOT-TRANSLATE placeholder is dropped, leaving 134; empty-message rows
// are also dropped.
//
...
struct LoadingTipEntry {
	std::string id;      // "E_Move", "GL_AccountDetails", "GP_...", ...
	std::string message; // the tip text (verbatim)
	int32_t order = 0;   // stable display order (file order)
};

class LoadingTipCatalog {
public:
	std::vector<LoadingTipEntry> items; // sorted by order
	...
	bool LoadFromJsonText(const std::string& text) {
		...
		for (const std::string& obj : SplitTopObjects(text)) {
			...
			r.message = Unescape(RawStr(obj, "message"));
			r.order   = (int32_t)RNum(obj, "order", 0);
			if (r.id.empty()) continue;
			auto it = std::find_if(items.begin(), items.end(),
				[&](const LoadingTipEntry& e){ return e.id == r.id; });
			if (it == items.end()) items.push_back(r);
			else *it = r;
			++touched;
		}
		std::sort(items.begin(), items.end(),
			[](const LoadingTipEntry& a, const LoadingTipEntry& b){ return a.order < b.order; });
		return touched > 0;
	}

	const LoadingTipEntry* Find(const std::string& id) const {
		for (const auto& r : items) if (r.id == id) return &r;
		return nullptr;
	}

	std::string Message(const std::string& id, const std::string& def = std::string()) const {
		const LoadingTipEntry* r = Find(id);
		return r ? r->message : def;
	}

	// All tips belonging to a category (first '_'-separated token: "GP", "SL", "UI", ...), in display order.
	std::vector<const LoadingTipEntry*> ForCategory(const std::string& category) const {
		std::vector<const LoadingTipEntry*> out;
	...
	// The category token: the first '_'-separated segment ("GL_AccountDetails" -> "GL"). Ids with no '_'
