#pragma once
// APB DISPLAY POINT catalog — the retail id -> {Title, ShortTitle, Description, ObtainedBy} dictionary for
// the progression/collectible/achievement entries surfaced in the UI (e.g. the "Graffiti - Empire Slipway"
// spray-tag collectibles found around each district, ranks, awards). Each point explains what it is and,
// via ObtainedBy, how the player earns it. Extracted from the retail DisplayPoint.INT (mirror of the cooked
// SDD table "DisplayPoint") by tools/scripts/extract_display_points.ps1 -> Content/Data/display_points.json.
//
// RETAIL DATA QUIRK (preserved 1:1): many entries were never given an authored ShortTitle and carry the
// literal placeholder "DNT - DO NOT TRANSLATE" in that field. This is the real retail string and is kept
// verbatim (do not "fix" it). The "None" id (a whole-row DNT placeholder) IS dropped. Of the 278 retail
// ids the "None" row is dropped, leaving 277.
//
...
struct DisplayPointEntry {
	std::string id;          // "Financial_Display_Graffiti_01", ...
	std::string title;       // full display name ("Graffiti - Empire Slipway, South")
	std::string short_title; // abbreviated name (often the retail "DNT - DO NOT TRANSLATE" placeholder)
	std::string description; // what it is
	std::string obtained_by; // how it is earned
	int32_t order = 0;       // stable display order (file order)
};

class DisplayPointCatalog {
public:
	std::vector<DisplayPointEntry> items; // sorted by order
	...
	bool LoadFromJsonText(const std::string& text) {
		...
		for (const std::string& obj : SplitTopObjects(text)) {
			...
			r.short_title = Unescape(RawStr(obj, "short_title"));
			r.description = Unescape(RawStr(obj, "description"));
			r.obtained_by = Unescape(RawStr(obj, "obtained_by"));
			r.order       = (int32_t)RNum(obj, "order", 0);
			if (r.id.empty()) continue;
			auto it = std::find_if(items.begin(), items.end(),
				[&](const DisplayPointEntry& e){ return e.id == r.id; });
			if (it == items.end()) items.push_back(r);
			else *it = r;
			++touched;
		}
		std::sort(items.begin(), items.end(),
			[](const DisplayPointEntry& a, const DisplayPointEntry& b){ return a.order < b.order; });
		return touched > 0;
	}

	const DisplayPointEntry* Find(const std::string& id) const {
		for (const auto& r : items) if (r.id == id) return &r;
		return nullptr;
	}

	std::string Title(const std::string& id, const std::string& def = std::string()) const {
		const DisplayPointEntry* r = Find(id);
		return r ? r->title : def;
	}

	std::string ShortTitle(const std::string& id, const std::string& def = std::string()) const {
		const DisplayPointEntry* r = Find(id);
		return r ? r->short_title : def;
	}
	...
	std::string ObtainedBy(const std::string& id, const std::string& def = std::string()) const {
		const DisplayPointEntry* r = Find(id);
		return r ? r->obtained_by : def;
	}

	// All display points belonging to a district / area (first '_'-separated token: "Financial",
	// "Waterfront", ...), in display order.
	std::vector<const DisplayPointEntry*> ForDistrict(const std::string& district) const {
		std::vector<const DisplayPointEntry*> out;
		for (const auto& r : items) if (District(r.id) == district) out.push_back(&r);
		return out;
	}

	int32_t Count() const { return (int32_t)items.size(); }

	// The district/area token: the first '_'-separated segment ("Financial_Display_..." -> "Financial").
	// Ids with no '_' return the id. Static.
	static std::string District(const std::string& id) {
