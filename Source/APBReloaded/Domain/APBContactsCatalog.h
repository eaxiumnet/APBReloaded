#pragma once
// APB CONTACT catalog — the AUTHORITATIVE retail id -> {display NAME, rich lore BIO} dictionary for the
// mission-giver NPCs that drive all of APB progression (Double-B, Veronika Lee, Britney Bloodrose,
// Grissom, ...). Extracted from the retail Contacts.INT (mirror of the cooked SDD table "Contacts") by
// tools/scripts/extract_contacts_catalog.ps1 -> Content/Data/contacts_catalog.json.
//
// PROVENANCE / WHY THIS EXISTS: the pre-existing Content/Data/contacts_lore.json is apbdb-scraped and its
// descriptions are TRUNCATED (~500 chars, cut mid-sentence). This catalog is the authoritative, UNTRUNCATED
// retail source (bios run to several thousand characters). It is a NEW, separate catalog; it does not touch
// contacts_lore.json or the ProgressionCatalog that consumes it, so the two can coexist merge-safely. Of
// the 98 retail contact ids the "None" DO-NOT-TRANSLATE placeholder is dropped, leaving 97; a few contacts
// carry a name but an empty bio (kept, since they still render a name).
//
...
struct ContactEntry {
	std::string id;          // "Financial_C1", "Waterfront_C3", "Social_Armas_Legendaries", ...
	std::string title;       // display name ("Double-B", "Britney Bloodrose", ...)
	std::string description; // multi-paragraph lore bio (verbatim; may be empty for a few contacts)
	int32_t order = 0;       // stable display order (file order)
};

class ContactCatalog {
public:
	std::vector<ContactEntry> items; // sorted by order
	...
	bool LoadFromJsonText(const std::string& text) {
		...
		for (const std::string& obj : SplitTopObjects(text)) {
			...
			auto it = std::find_if(items.begin(), items.end(),
				[&](const ContactEntry& e){ return e.id == r.id; });
			if (it == items.end()) items.push_back(r);
			else *it = r;
			++touched;
		}
		std::sort(items.begin(), items.end(),
			[](const ContactEntry& a, const ContactEntry& b){ return a.order < b.order; });
		return touched > 0;
	}

	const ContactEntry* Find(const std::string& id) const {
		for (const auto& r : items) if (r.id == id) return &r;
		return nullptr;
	}

	std::string Title(const std::string& id, const std::string& def = std::string()) const {
		const ContactEntry* r = Find(id);
	...
	// True if the contact exists AND carries a non-empty bio (a few contacts have a name but no bio).
	bool HasDescription(const std::string& id) const {
		const ContactEntry* r = Find(id);
		return r && !r->description.empty();
	}

	// All contacts belonging to a district / area (first '_'-separated token: "Financial", "Waterfront",
	// "Social", ...), in display order.
	std::vector<const ContactEntry*> ForDistrict(const std::string& district) const {
		std::vector<const ContactEntry*> out;
	...
	// The district/area token: the first '_'-separated segment ("Financial_C1" -> "Financial"). Ids with
