#pragma once
// APB scoreboard-column catalog (post-mission + chaos end-of-round scoreboard column tooltips),
// extracted from the retail ScoreboardDescriptions.INT (mirror of the cooked SDD table
// "ScoreboardDescription") by tools/scripts/extract_scoreboard_descriptions.ps1 ->
// Content/Data/scoreboard_descriptions.json.
//
// Header-only (matches MedalCatalog / StreetNameCatalog / AmmoCategoryCatalog): every method is
// defined in-class so it is implicitly inline and safe to include in multiple TUs.
//
// Each entry is a scoreboard column key + its hover/tooltip description, both verbatim from the
// INT:
//   id            column key ("Arrests", "Kills", "Deaths", "Score", "Threat", ...)
//   display_text  tooltip text shown for that column on the scoreboard
#include <string>
#include <vector>
#include <fstream>
#include <sstream>
...
struct ScoreboardDescription {
	std::string id;           // "Arrests", "Kills", "Score", "Threat", ...
	std::string display_text; // "Number of arrests during the mission", ...
	int32_t order = 0;        // stable display order (file order)
};

class ScoreboardDescriptionCatalog {
public:
	std::vector<ScoreboardDescription> columns; // sorted by order
	...
	bool LoadFromJsonText(const std::string& text) {
		...
		for (const std::string& obj : SplitTopObjects(text)) {
			ScoreboardDescription c;
			c.id           = Unescape(RawStr(obj, "id"));
			c.display_text = Unescape(RawStr(obj, "display_text"));
			c.order        = (int32_t)RNum(obj, "order", 0);
			if (c.id.empty()) continue;
			auto it = std::find_if(columns.begin(), columns.end(),
				[&](const ScoreboardDescription& e){ return e.id == c.id; });
			if (it == columns.end()) columns.push_back(c);
			else *it = c;
			++touched;
		}
		std::sort(columns.begin(), columns.end(),
			[](const ScoreboardDescription& a, const ScoreboardDescription& b){ return a.order < b.order; });
		return touched > 0;
	}

	const ScoreboardDescription* Find(const std::string& id) const {
		for (const auto& c : columns) if (c.id == id) return &c;
		return nullptr;
	}

	std::string DisplayText(const std::string& id, const std::string& def = std::string()) const {
		const ScoreboardDescription* c = Find(id);
		return c ? c->display_text : def;
	}

	int32_t Count() const { return (int32_t)columns.size(); }
