#pragma once
// APB CLOTHING item-type catalog — the id -> player-facing DESCRIPTION dictionary the Armas marketplace /
// character-customization / inventory UI shows as a clothing item's flavour + role blurb (e.g. armoured
// arm pads, faction jackets, preset outfits). Extracted from the retail ClothingItemTypes.INT (mirror of
// the cooked SDD table "ClothingItemTypes") by tools/scripts/extract_clothing_item_types.ps1 ->
// Content/Data/clothing_item_types.json.
//
// This is the DESCRIPTION leg of the clothing-info pairing:
//   - (apbdb clothing catalogs) -> slot / faction / unlock metadata
//   - clothing_item_types.json  (THIS) -> id -> rich description
// Of the ~2058 clothing ids only ~1836 carry a description (empty-description rows are dropped). It is the
// largest of the ItemTypes description tables and core to APB character customization.
//
...
struct ClothingItemType {
	std::string id;          // "Clothing_F_Armpads_Armoured", "Clothing_Preset_...", "Clothing_M_...", ...
...
class ClothingItemTypeCatalog {
public:
	std::vector<ClothingItemType> items; // sorted by order
	...
	bool LoadFromJsonText(const std::string& text) {
		...
		for (const std::string& obj : SplitTopObjects(text)) {
			...
			auto it = std::find_if(items.begin(), items.end(),
				[&](const ClothingItemType& e){ return e.id == r.id; });
			if (it == items.end()) items.push_back(r);
			else *it = r;
			++touched;
		}
		std::sort(items.begin(), items.end(),
			[](const ClothingItemType& a, const ClothingItemType& b){ return a.order < b.order; });
		return touched > 0;
	}

	const ClothingItemType* Find(const std::string& id) const {
	...
	std::string Description(const std::string& id, const std::string& def = std::string()) const {
		const ClothingItemType* r = Find(id);
		return r ? r->description : def;
	}

	// True if the clothing item exists (every stored row has a non-empty description).
	bool HasDescription(const std::string& id) const {
		const ClothingItemType* r = Find(id);
		return r && !r->description.empty();
	}

	// All clothing whose id belongs to a family (first '_'-separated token), in display order.
	std::vector<const ClothingItemType*> ForCategory(const std::string& category) const {
		std::vector<const ClothingItemType*> out;
		for (const auto& r : items) if (Category(r.id) == category) out.push_back(&r);
		return out;
	}

	// All clothing of a CLASS (second '_'-separated token: "F"/"M" gender-slot items or "Preset" outfits),
	// in display order. For clothing ids the first token is (almost always) "Clothing", so the class token
	// is the discriminating one.
	std::vector<const ClothingItemType*> ForClass(const std::string& clothingClass) const {
		std::vector<const ClothingItemType*> out;
		for (const auto& r : items) if (Class(r.id) == clothingClass) out.push_back(&r);
		return out;
	}

	int32_t Count() const { return (int32_t)items.size(); }

	// The family token: the first '_'-separated segment ("Clothing_F_Armpads_Armoured" -> "Clothing").
	// Ids with no '_' return the id. Static. NOTE: two retail rows carry a "Cloting_" typo prefix (a
	// genuine source-data quirk, preserved verbatim), so this is not guaranteed to be "Clothing".
	...
	// The clothing CLASS token: the second '_'-separated segment ("Clothing_F_Armpads_Armoured" -> "F";
	// "Clothing_Preset_..." -> "Preset"). Ids with fewer than two segments return an empty string. Static.
