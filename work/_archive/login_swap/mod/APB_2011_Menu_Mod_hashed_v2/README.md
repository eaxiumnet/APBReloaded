# APB 2011 Main Menu Texture Mod (Hash-Prebuilt v2)
...
**v2 improvements:**
- Hashes are computed using the correct locked-surface byte layout (BGRA for RGBA textures, BGR for RGB textures).
- Includes fallback hashes for multiple common D3D9 layouts (RGBA, BGR, RGB) in case the runtime format differs.
- Better handling of mixed-mode retail textures.
...
uMod computes its hash from the **locked D3D9 surface**. APB may store menu textures in DXT-compressed or other GPU-native formats. UE Viewer exports decompressed TGA images, but uMod hashes the raw locked bytes, which may be DXT blocks. This means pre-computed hashes from TGA exports are a best-effort starting point and may not match at runtime.

**The reliable path is to capture live hashes with uMod.** See `MANUAL_CAPTURE_GUIDE.md` and use `capture_and_build.ps1` in this folder.

## If the pre-hashed mod does not work

Use the included `capture_and_build.ps1` script:
...
See `MANUAL_CAPTURE_GUIDE.md` for detailed instructions.
...
- ✅ Retail APB Reloaded log confirms Direct3D 9 mode:
