...
This version loads the template AFTER adding the game, since uMod's settings
are per-game.
"""
import subprocess
import time
...
def click_menu_item(umod_win, menu_index, item_index):
    """Click a menu item by approximate coordinates."""
    pyautogui.click(umod_win.left + 30, umod_win.top + 30)
    time.sleep(0.5)
    pyautogui.click(umod_win.left + 60 + (menu_index * 0), umod_win.top + 80 + (item_index * 20))
    time.sleep(0.5)
...
def main():
    ...
    if not umod_win:
        ...
        return
    print(f"uMod window: {umod_win.title} at ({umod_win.left}, {umod_win.top})")
    umod_win.activate()
    time.sleep(1)

    # Add game
    print("Adding APB.exe...")
    click_menu_item(umod_win, 0, 2)
    time.sleep(2)
